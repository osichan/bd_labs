...
def _init_swagger(app: Flask) -> None:
    # A-lia Swagger
    restx_api = Api(app, title="Saienko Oskar lab backend", description="A simple backend")
...
def _process_input_config(
    ...
    app_config[SQLALCHEMY_DATABASE_URI] = (
        f"mysql://{root_user}:{root_password}@"
        f"{app_config['MYSQL_HOST']}:{app_config['MYSQL_PORT']}/"
        f"{app_config['MYSQL_DATABASE']}"
    )
    print(f"Database URL: {app_config[SQLALCHEMY_DATABASE_URI]}")
